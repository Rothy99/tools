...
export const TextDiffTool: React.FC = () => {
  const [textA, setTextA] = useSessionStorageString("devstudio_text_diff_a", SAMPLE_PRESETS.textDiffA);
  const [textB, setTextB] = useSessionStorageString("devstudio_text_diff_b", SAMPLE_PRESETS.textDiffB);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex flex-col bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-2xs">
          ...
          <textarea
            ...
            className="w-full p-4 font-mono text-xs bg-transparent text-slate-900 dark:text-slate-100 focus:outline-hidden resize-none leading-relaxed h-[700px]"
          />
        </div>
      ...
      <div className="bg-slate-950 rounded-2xl border border-slate-800 overflow-hidden shadow-2xs">
            ...
            <button
              onClick={() => {
                const diffText = diffLines
                  .map((l) => `${l.type === "added" ? "+" : l.type === "removed" ? "-" : " "}${l.contentB ?? l.contentA}`)
                  .join("\n");
                navigator.clipboard.writeText(diffText);
              }}
              className="flex items-center gap-1 text-[11px] font-medium text-indigo-400 hover:text-indigo-300 cursor-pointer"
            >
              <Copy className="w-3 h-3" /> Copy Unified Diff
            </button>
        ...
        <div className="p-4 font-mono text-xs overflow-x-auto min-h-[700px] max-h-[1100px]">
